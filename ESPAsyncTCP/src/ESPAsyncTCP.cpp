...
#if ASYNC_TCP_SSL_ENABLED
bool AsyncClient::connect(IPAddress ip, uint16_t port, bool secure=false)
#else
...
ip_addr_t AsyncClient::getRemoteAddress6() {
    if(!_pcb) {
		ip_addr_t nulladdr = {0};
		return nulladdr;
        /*ip6_addr_t nulladdr;
...
ip_addr_t AsyncClient::getLocalAddress6() {
    if(!_pcb) {
		ip_addr_t nulladdr = {0};
		return nulladdr;
        /*ip6_addr_t nulladdr;
...
ip_addr_t AsyncClient::remoteIP6() {
    return getRemoteAddress6();
}
...
ip_addr_t AsyncClient::localIP6() {
    return getLocalAddress6();
}
...
void AsyncServer::begin(){
  ...
  tcp_setprio(pcb, TCP_PRIO_MIN);
  ip_addr_t local_addr;
  local_addr.u_addr.ip6.addr = _addr;
  ...
  tcp_pcb* listen_pcb = tcp_listen(_pcb);
  if (!listen_pcb) {
    tcp_close(_pcb);
    return;
  }
